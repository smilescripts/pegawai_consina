...
<ol class="breadcrumb" id="non-printable">
  ...
  <li class="active"><span class="glyphicon glyphicon-user">Rekapitulasi Absensi Kehadiran Karyawan</li>
</ol>
...
<div class="panel panel-warning" id="non-printable">
  <h1 class="headingtable" style="margin-top:0px" ><span>Rekapitulasi</span>Absensi Kehadiran Karyawan</h1>
		...
		<div class="well">
            <form class="form-horizontal rekapitulasi_bulananForm" id="rekapitulasi_bulananForm" action="modul/mod_rekapitulasi_absensi/rekapitulasi_absensi.php" type="POST">
				<div class="form-group">
					...
					<option value="Kontrak">Harian (Jakarta)</option>
					<option value="Kontrak Bekasi">Harian (Bekasi)</option>
					<option value="Tetap">Bulanan (Office)</option>
					<option value="TetapOutlet">Bulanan (Counter)</option>
					</select>
                    </div>
					...
					.on('success.form.fv', function(e) {
						...
						$.ajax({
							...
							success: function(data) {
								...
								if(format=="Tetap"){
								$.get("modul/mod_rekapitulasi_absensi/absensi_office.php?start="+start+"&end="+end+"&DEPT="+DEPT+"&NIP_PEGAWAIH="+NIP_PEGAWAIH, function(data){
								
									$('#laporan').html(data);
									$('#loadingDiv').hide(0);
								});
								}
								if(format=="TetapOutlet"){
								$.get("modul/mod_rekapitulasi_absensi/absensi_outlet.php?start="+start+"&end="+end+"&DEPT="+DEPT+"&NIP_PEGAWAIH="+NIP_PEGAWAIH, function(data){
								...
								if(format=="Kontrak"){
								$.get("modul/mod_rekapitulasi_absensi/absensi_harian_jakarta.php?start="+start+"&end="+end+"&DEPT="+DEPT+"&NIP_PEGAWAIH="+NIP_PEGAWAIH, function(data){
								
									$('#laporan').html(data);
									$('#loadingDiv').hide(0);
								});
								}
								if(format=="Kontrak Bekasi"){
								$.get("modul/mod_rekapitulasi_absensi/absensi_harian_bekasi.php?start="+start+"&end="+end+"&DEPT="+DEPT+"&NIP_PEGAWAIH="+NIP_PEGAWAIH, function(data){
...
<div class="panel panel-warning">
    <h1 class="headingtable" style="margin-top:0px" ><span>Data Rekapitulasi</span> Rekapitulasi Absensi</h1>
    <div class="panel-body">
	<center>
