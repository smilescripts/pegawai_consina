...
<div class="panel panel-warning" id="non-printable">
	...
	<?php
		...
		if($DEPT=="all" && $NIP_PEGAWAIH!=""){
			...
			$query=mysql_query("SELECT * FROM pegawai WHERE STATUS_PEGAWAI='Tetap' and KODE_PEGAWAI='$pegawaicari->KODE_PEGAWAI' and OUTLET='YA'") or die (mysql_error());
	...
	<center><h3><u>REKAPITULASI KEHADIRAN DAN GAJI KARYAWAN BULANAN</u></h3><h3><?php echo $namabulan;?> <?php echo $TAHUN;?></h3></center>
		...
		<table  class="table table-bordered" id="printable">
			...
			<?php 
				...
				while($getabsensidata=mysql_fetch_object($absensidata)){
					...
					$datapegawai=mysql_query("SELECT * FROM pegawai where KODE_PEGAWAI='$objectdata->KODE_PEGAWAI' and OUTLET='YA'") or die (mysql_error());	
		...
		<div class="col-md-6">
			...
			<?php
				...
				$total_potongan=number_format($kasbon+$nominaltabungan+$nominalpinjaman+$terlambat);
				$total_penerimaan=number_format(getthp($NIP) + $uang_makan_transport + $nominal_kehadiran_full+$lembur);
				$takehomepayfix2=(getthp($NIP) + $uang_makan_transport + $nominal_kehadiran_full+$lembur)-($kasbon+$nominaltabungan+$nominalpinjaman+$terlambat);
			?>
